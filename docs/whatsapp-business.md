...
3. **Botón en `/book`:** poner el número en `app/src/lib/business.ts` →
   `export const WHATSAPP_NUMBER = "34XXXXXXXXX";` → push a `master`.
4. **(Opcional) Footer en el email:** `supabase secrets set WHATSAPP_NUMBER=34XXXXXXXXX`.
   Si no lo pones, el email frío sale limpio como hasta ahora.
