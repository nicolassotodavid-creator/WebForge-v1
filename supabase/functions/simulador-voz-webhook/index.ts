// simulador-voz-webhook — [SIMULADOR] ElevenLabs manda aquí el resumen de cada conversación del asistente de
// voz de las demos (agente "Simulador — ventas", docs/prospeccion/agente-simulador/). El agente tiene este webhook
// como override del de la cuenta, así que sus conversaciones no llegan al post-call de Luvia (ni al revés).
//
// Por cada conversación:
//  · verifica la firma HMAC (cabecera ElevenLabs-Signature, secreto SIMULADOR_VOZ_WEBHOOK_SECRET);
//  · la guarda en events (type 'simulador_voz'), con lead_id si la empresa casa con un lead;
//  · avisa a Nico por email (SIMULADOR_AVISO_EMAIL) con lo que recogió el agente: nombre, email, WhatsApp,
//    si le interesa y qué le frena. Un "quiere montarlo" hay que contestarlo en el día (speed-to-lead).
//
...
const AGENT_ID = "agent_3501m3e6ckapepvt1y8tags6ct7t";
...
Deno.serve(async (req: Request) => {
  ...
  if (!(await firmaValida(rawBody, req.headers.get("elevenlabs-signature"), Deno.env.get("SIMULADOR_VOZ_WEBHOOK_SECRET")))) {
  ...
  const empresa: string = data.conversation_initiation_client_data?.dynamic_variables?.empresa ?? "(empresa desconocida)";
  ...
  // Lead por nombre de empresa (el nombre de la demo suele estar contenido en el de Maps). Si hay varios, ninguno.
  let leadId: string | null = null;
  if (empresa && !empresa.startsWith("(")) {
    const { data: leads } = await supabase.from("leads").select("id").ilike("name", `%${empresa.replace(/[%_]/g, "")}%`).limit(2);
  ...
  if (errEvento) console.error("[simulador-voz] no se pudo guardar el evento:", errEvento.message);

  // Aviso a Nico.
  const destino = Deno.env.get("SIMULADOR_AVISO_EMAIL");
  const resendKey = Deno.env.get("RESEND_API_KEY");
  if (!destino || !resendKey) {
    console.error("[simulador-voz] falta SIMULADOR_AVISO_EMAIL o RESEND_API_KEY: no se avisa");
    return json({ ok: true, avisado: false });
  }
  const quiere = interesado === true && (email || whatsapp);
  const asunto = quiere
    ? `🔥 Simulador: ${empresa} quiere montarlo`
    : !hablo
    ? `Simulador: ${empresa} abrió el asistente y no habló`
    : interesado === false
    ? `Simulador: ${empresa} habló con el asistente (no le interesa)`
    : `Simulador: ${empresa} habló con el asistente`;
  ...
  const lineas = [
    ...
    `Qué le frena: ${objecion ?? "—"}`,
    `Duración: ${Math.round(duracion)} s`,
    "",
    resumen ? `Resumen: ${resumen}` : "",
    quiere ? "\nContéstale hoy por WhatsApp para montárselo." : "",
    "",
    "Conversación:",
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Simulador <hola@nico-soto.es>",
      to: [destino],
      subject: asunto,
      text: texto,
  ...
  if (!res.ok) console.error("[simulador-voz] Resend", res.status, (await res.text()).slice(0, 300));
