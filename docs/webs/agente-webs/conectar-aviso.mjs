// [WEBS] Conecta el asistente de voz de la portada con la función webs-voz-webhook (aviso por email a Nico).
// 1) Crea en ElevenLabs un webhook de workspace "Webs post-call → WebForge" con firma HMAC.
// 2) Guarda su secreto en Supabase (WEBS_VOZ_WEBHOOK_SECRET) y el destinatario del aviso
//    (WEBS_AVISO_EMAIL), vía Management API con SUPABASE_ACCESS_TOKEN. El secreto no se imprime.
// 3) Pone ese webhook como override SOLO del agente de webs: el webhook de la cuenta (el de Luvia)
//    sigue igual para los agentes de Luvia.
// Uso: node docs/webs/agente-webs/conectar-aviso.mjs [email-destino]  (sin destino usa SIMULADOR_AVISO_EMAIL)
...
const NOMBRE = "Webs post-call → WebForge";
const URL_FN = `https://${REF}.supabase.co/functions/v1/webs-voz-webhook`;
...
if (webhookId) {
  ...
  const s = await fetch(`https://api.supabase.com/v1/projects/${REF}/secrets`, {
    ...
    body: JSON.stringify([
      { name: "WEBS_VOZ_WEBHOOK_SECRET", value: b.webhook_secret },
      ...(DESTINO ? [{ name: "WEBS_AVISO_EMAIL", value: DESTINO }] : []),
    ]),
  });
