// [SIMULADOR] Lote 1 (1-15, ya contactados por formulario el 25-ago) por email, desde hola@nico-soto.es vía Resend.
// Fuente: outreach_reformas_lote1.csv (16-sep, asunto/cuerpo/seguimiento por cluster). {link_demo} = demo_url de home-estimator-outreach-15.csv.
// Robles Home (9) no va: su seguimiento es aparte.
// Uso: node docs/prospeccion/enviar-email-lote1.mjs                          → simulación
//      node docs/prospeccion/enviar-email-lote1.mjs --enviar                 → email 1
//      node docs/prospeccion/enviar-email-lote1.mjs --seguimiento --enviar   → email 2 (seguimiento)
//      node docs/prospeccion/enviar-email-lote1.mjs [--seguimiento] --registrar → solo apunta en el panel lo ya enviado (log)
...
const cola = parseCsv(fs.readFileSync(path.join(DIR, "home-estimator-outreach-15.csv"), "utf8"), ";");
const demoPorEmail = new Map(cola.map((r) => [r.email.trim().toLowerCase(), r]));

const LEADS = parseCsv(fs.readFileSync(path.join(DIR, "outreach_reformas_lote1.csv"), "utf8"), ",")
  .filter((r) => r.enviar === "TRUE")
  .map((r) => {
    const c = demoPorEmail.get(r.email.trim().toLowerCase());
...
const LOG = path.join(DIR, `home-estimator-lote1-${SEGUIMIENTO ? "seguimiento" : "email1"}-envios.json`);
