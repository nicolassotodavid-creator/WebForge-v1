...
const CFG = { webforge: "hello@nico-soto.es", luvia: "marketing@luvia-ia.es" };

assertEq(replyToFor(MIGUEL, ADMIN, CFG), "marketing@luvia-ia.es", "lead Luvia → buzón Luvia");
assertEq(replyToFor(ADMIN, ADMIN, CFG), "hello@nico-soto.es", "lead admin → buzón WebForge");
assertEq(replyToFor(null, ADMIN, CFG), "hello@nico-soto.es", "lead sin dueño (cron) → buzón WebForge");
assertEq(
  replyToFor(MIGUEL, ADMIN, { webforge: "hello@nico-soto.es", luvia: "  " }),
  undefined,
